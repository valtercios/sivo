...
<head>
    ...
    <title>ENCAMINHAMENTO A DEFENSORIA</title>
...
<body>
    <div id="main">
        ...
        <div class="subtitulo">
            <h4>ENCAMINHAMENTO Á DEFENSORIA</h4>
        </div>
        ...
        <div id="conteudo">
            ...
            <p>Á Defensoria Pública do Estado do Rio Grande do Norte</p>
            <p>Encaminhamos o Sr.(a) <strong>{{ $corpo->responsavelCorpo->nome ?? $corpo->responsavelEntrega->nome }}</strong></p>
            <p>identidade nº <strong>{{ $corpo->responsavelCorpo->rg ?? $corpo->responsavelEntrega->rg }}</strong>, a fim de requerer Alvará Judicial para autorização
                de necropsia e retirada do corpo, sepultamento e registro em cartório, de <strong>{{ $corpo->nome }}</strong>, que deu entrada neste
            serviço em {{ \Carbon\Carbon::parse($corpo->data_entrada)->format('d/m/Y') }}</p>
            <br>
            <br>
            A solicitação se dá, em virtude:

            <pre class="area-texto">{{ $dados['informacoes_adicionais'] ?? '' }}</pre>
