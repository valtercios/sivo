...
<body>
    <div id="main">
        ...
        <div id="conteudo">
            <p>
                À (ao) {{ strtoupper($dados['encaminharquem']) }}<br>
                Encaminhamos o(a)<br>
                Sr(a) <strong>{{ $corpo->responsavelCorpo->nome ?? $corpo->responsavelEntrega->nome }}</strong><br>
                RG: <strong>{{ $corpo->responsavelCorpo->rg ?? $corpo->responsavelEntrega->rg }}</strong> CPF:
                <strong>{{ $corpo->responsavelCorpo->cpf ?? $corpo->responsavelEntrega->cpf }}</strong>, responsável pelo corpo
                de <strong>{{ $corpo->nome }}</strong> nascido em
                <strong>{{ \Carbon\Carbon::parse($dados['data_nascimento_corpo'])->format('d/m/Y') }}</strong>
                e
                falecido na data de <strong>{{ \Carbon\Carbon::parse($corpo->data_obito)->format('d/m/Y') }}</strong>,
                cujo
                corpo encontra-se no
                Serviço de Verificação de Óbitos de Natal.<br><br>

                Motivo do encaminhamento: <br>
                <pre class="area-texto">{{ $dados['motivo_encaminhamento'] }}</pre>
                <br>
